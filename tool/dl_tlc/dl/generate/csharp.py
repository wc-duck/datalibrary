...
def emit_member( member, mem_ctx ):
    ...
    if isinstance( member, tl.InlineArrayMember ):
        if isinstance( member.type, tl.BuiltinType ) or isinstance( member.type, tl.Enum ):
            lines.append( '[MarshalAsAttribute(UnmanagedType.ByVal%s, SizeConst=%u)]' % ( 'TStr' if member.type == 'string' else 'Array', member.count ) ) 
            lines.append( 'public %s[] %s = new %s[%u];' % ( cs_name, member.name, cs_name, member.count ) )
        else:
            lines.append( '[MarshalAsAttribute(UnmanagedType.ByVal%s, SizeConst=%u)]' % ( 'TStr' if member.type == 'string' else 'Array', member.count ) ) 
            lines.append( 'public %s[] %s = DLHelper.InitArray<%s>(%u);' % ( cs_name, member.name, cs_name, member.count ) )
    ...
    elif isinstance( member.type, tl.Enum ):
        lines.append( 'public %s %s;' % ( cs_name, member.name ) )
        ...
        if member.last_in_group:
            ...
            mem_ctx.bf_group += 1
    elif isinstance( member.type, tl.BuiltinType ):
        lines.append( 'public %s %s;' % ( cs_name, member.name ) )
    elif isinstance( member, tl.PodMember ):
