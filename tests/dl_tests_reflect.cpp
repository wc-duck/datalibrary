...
TEST_F(DL, ReflectPods)
{
	...
	EDLError err = DLReflectGetTypeInfo(Ctx, SPods::TYPE_ID, &Info, Members, M_ARRAY_LENGTH(Members));
...
TEST_F(DL, SizeAndAlignment)
{
	...
	EXPECT_EQ(M_ALIGNMENTOF(SPods2),                       DLAlignmentOfType(Ctx, SPods2::TYPE_ID));
	EXPECT_EQ(sizeof(SPtrChain),                           DLSizeOfType(Ctx, SPtrChain::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SPtrChain),                    DLAlignmentOfType(Ctx, SPtrChain::TYPE_ID));
	EXPECT_EQ(sizeof(SPods),                               DLSizeOfType(Ctx, SPods::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SPods),                        DLAlignmentOfType(Ctx, SPods::TYPE_ID));
	EXPECT_EQ(sizeof(SMoreBits),                           DLSizeOfType(Ctx, SMoreBits::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SMoreBits),                    DLAlignmentOfType(Ctx, SMoreBits::TYPE_ID));
	EXPECT_EQ(sizeof(SMorePods),                           DLSizeOfType(Ctx, SMorePods::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SMorePods),                    DLAlignmentOfType(Ctx, SMorePods::TYPE_ID));
	EXPECT_EQ(sizeof(SStrings),                            DLSizeOfType(Ctx, SStrings::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SStrings),                     DLAlignmentOfType(Ctx, SStrings::TYPE_ID));
	EXPECT_EQ(sizeof(STestBits),                           DLSizeOfType(Ctx, STestBits::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(STestBits),                    DLAlignmentOfType(Ctx, STestBits::TYPE_ID));
	EXPECT_EQ(sizeof(SPodArray1),                          DLSizeOfType(Ctx, SPodArray1::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SPodArray1),                   DLAlignmentOfType(Ctx, SPodArray1::TYPE_ID));
	EXPECT_EQ(sizeof(SPodArray2),                          DLSizeOfType(Ctx, SPodArray2::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SPodArray2),                   DLAlignmentOfType(Ctx, SPodArray2::TYPE_ID));
	EXPECT_EQ(sizeof(SSimplePtr),                          DLSizeOfType(Ctx, SSimplePtr::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SSimplePtr),                   DLAlignmentOfType(Ctx, SSimplePtr::TYPE_ID));
	EXPECT_EQ(sizeof(SStringArray),                        DLSizeOfType(Ctx, SStringArray::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SStringArray),                 DLAlignmentOfType(Ctx, SStringArray::TYPE_ID));
	EXPECT_EQ(sizeof(SStructArray1),                       DLSizeOfType(Ctx, SStructArray1::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SStructArray1),                DLAlignmentOfType(Ctx, SStructArray1::TYPE_ID));
	EXPECT_EQ(sizeof(SPod2InStruct),                       DLSizeOfType(Ctx, SPod2InStruct::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SPod2InStruct),                DLAlignmentOfType(Ctx, SPod2InStruct::TYPE_ID));
	EXPECT_EQ(sizeof(SWithInlineArray),                    DLSizeOfType(Ctx, SWithInlineArray::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SWithInlineArray),             DLAlignmentOfType(Ctx, SWithInlineArray::TYPE_ID));
	EXPECT_EQ(sizeof(SStringInlineArray),                  DLSizeOfType(Ctx, SStringInlineArray::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SStringInlineArray),           DLAlignmentOfType(Ctx, SStringInlineArray::TYPE_ID));
	EXPECT_EQ(sizeof(SPod2InStructInStruct),               DLSizeOfType(Ctx, SPod2InStructInStruct::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SPod2InStructInStruct),        DLAlignmentOfType(Ctx, SPod2InStructInStruct::TYPE_ID));
	EXPECT_EQ(sizeof(SWithInlineStructArray),              DLSizeOfType(Ctx, SWithInlineStructArray::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SWithInlineStructArray),       DLAlignmentOfType(Ctx, SWithInlineStructArray::TYPE_ID));
	EXPECT_EQ(sizeof(SWithInlineStructStructArray),        DLSizeOfType(Ctx, SWithInlineStructStructArray::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SWithInlineStructStructArray), DLAlignmentOfType(Ctx, SWithInlineStructStructArray::TYPE_ID));
	EXPECT_EQ(sizeof(SDoublePtrChain),                     DLSizeOfType(Ctx, SDoublePtrChain::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SDoublePtrChain),              DLAlignmentOfType(Ctx, SDoublePtrChain::TYPE_ID));
	EXPECT_EQ(sizeof(SA128BitAlignedType),                 DLSizeOfType(Ctx, SA128BitAlignedType::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SA128BitAlignedType),          DLAlignmentOfType(Ctx, SA128BitAlignedType::TYPE_ID));

	EXPECT_EQ(sizeof(SBugTest1),                DLSizeOfType(Ctx, SBugTest1::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SBugTest1),         DLAlignmentOfType(Ctx, SBugTest1::TYPE_ID));
	EXPECT_EQ(sizeof(SBugTest1_InArray),        DLSizeOfType(Ctx, SBugTest1_InArray::TYPE_ID));
	EXPECT_EQ(M_ALIGNMENTOF(SBugTest1_InArray), DLAlignmentOfType(Ctx, SBugTest1_InArray::TYPE_ID));
}
